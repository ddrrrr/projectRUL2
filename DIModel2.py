...
class Process():
    def __init__(self):
        ...
        # self.network = CDC(16,self.batch_size,self.seq_len,self.fea_ size).cuda()
    ...
    def _cal_one_batch(self, data_iter, isTrain=False, opti=None,train_idx=None):
        ...
        for i,x in enumerate(each_data_num):
            ...
            for one_idx in temp_idx:
                if isTrain:
                    temp_jiange = np.random.randint(1,one_idx//self.seq_len+1)
                else:
                    temp_jiange=1
                batch_data.append(data_iter[i][one_idx-self.seq_len*temp_jiange:one_idx:temp_jiange,:,:].reshape([1,2,self.seq_len,data_len]))
    ...
    def _gen_feature(self, model_name):
        ...
        for i in range(len(bearing_data)):
            ...
            for j in range(temp_one_data.shape[0]//64+1):
                temp_one_fea = feature_net(temp_one_data[j*64:min(j*64+64,temp_one_data.shape[0]-1)].unsqueeze(1))
